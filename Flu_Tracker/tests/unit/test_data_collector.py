...
class DataCollectorTests(unittest.TestCase):
    ...
    def test_record_map_point_calls_database_handler(self, mock_write_map_point):
        ...
        expected_args = {'date': int(self.mock_timestamp), 'lat':self.mock_latitude, 'long':self.mock_longitude, 'text':self.valid_text_mock}

        self.test_listener.record_map_point(self.mock_latitude, self.mock_longitude, self.mock_timestamp, self.valid_text_mock)
